...
- Fail-fast semantic: if the auto-register call raises (network error, server unavailable, name conflict surfacing as `HubTransientError`, etc.), `AgentHub.connect()` propagates the exception and tears down the just-opened MCP session before the caller's `async with` body runs. Same spirit as `ConfigurationError` for unresolved URL/PAT (redline #1) — no half-open handles leak. Python: the surrounding `HubSession.open` `asynccontextmanager` handles cleanup; TS: `AgentHub.connect` explicitly calls `handle[Symbol.asyncDispose]()` before re-throwing.
- Subsequent calls to `hub.register(...)` (Python) / `hub.session.register(...)` (TS) remain supported and pass through to the server. The agent-hub `register` tool is idempotent server-side, so a manual re-register is a harmless re-confirm or an explicit `display_name` update if the field changed. The SDK does not gate duplicates.
- Mode-agnostic: `stateful` / `stateless` / `global` all auto-register with the declared mode. No opt-out flag — if a future global-mode plugin needs to skip register, an opt-out hook can be added then (YAGNI).
- `display_name` fallback: when `display_name` is `None` at connect time, the auto-register call uses `user` as the display name, mirroring the existing `HubSession.register()` fallback semantic (M1).
